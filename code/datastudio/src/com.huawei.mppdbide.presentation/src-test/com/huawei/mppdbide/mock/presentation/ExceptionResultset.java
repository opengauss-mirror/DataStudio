...
public class ExceptionResultset implements ResultSet
{
    ...
    public boolean next() throws SQLException
    {
        if (EXCEPTIONENUM.EXCEPTION.equals(resultSetNext))
        {
            ...
            {
                if("SELECT c.oid, n.oid as nspoid, n.nspname AS schemaname, c.relname AS viewname, pg_get_userbyid(c.relowner) AS viewowner FROM (pg_class c LEFT JOIN pg_namespace n ON ((n.oid = c.relnamespace))) WHERE (c.relkind = 'v'::\"char\" or c.relkind = 'm'::\"char\") and has_table_privilege(c.oid,'SELECT') ".equals(exceptioncounter))
                if(null != sqlState && sqlState.length() > 0)
                {
                    throw new SQLException(sqlState, sqlState);
                }
