 ...
 * Title: class
 * 
 * Description: The Class CommandUnIndentation.
 *
 * @since 3.0.0
 */
public class CommandUnIndentation {
    ...
    public void execute(@Named(IServiceConstants.ACTIVE_PART) @Optional MPart activePart) {
        ...
        if (partObject instanceof PLSourceEditor) {
            ((PLSourceEditor) partObject).getSourceEditorCore()
                    .performIndentUnindentOperation(PLSourceEditorCore.UNINDENT_OPERATION, null);
    ...
    public boolean canExecute(@Named(IServiceConstants.ACTIVE_PART) @Optional MPart activePart) {
        ...
        if (partObject instanceof SQLTerminal) {
            ...
            return terminal.getTerminalCore().getSelectedLineCount()
