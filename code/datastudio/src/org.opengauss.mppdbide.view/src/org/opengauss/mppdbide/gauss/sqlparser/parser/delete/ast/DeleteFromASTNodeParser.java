...
package org.opengauss.mppdbide.gauss.sqlparser.parser.delete.ast;
...
/**
 * 
 * Title: DeleteFromASTNodeParser
 *
 * @since 3.0.0
 */
public class DeleteFromASTNodeParser extends BasicASTNodeParser {
    ...
    protected void prepareASTOtherStmtObject(ListIterator<ISQLTokenData> listIterator, TBasicASTNode fromAstNode) {
        ...
        TDeleteFromASTNode groupByAstNode = (TDeleteFromASTNode) fromAstNode;

        TSqlNode only = ParserUtils.handleToken(listIterator, SQLFoldingConstants.SQL_KEYWORD_ONLY);
        groupByAstNode.setOnly(only);

        TSqlNode from = ParserUtils.handleToken(listIterator, SQLFoldingConstants.SQL_KEYWORD_FROM);
        groupByAstNode.setFrom(from);

        if (null == from) {
            setTableName(listIterator, groupByAstNode);
            return;
        } else {
    ...
    private void setTableName(ListIterator<ISQLTokenData> listIterator, TDeleteFromASTNode groupByAstNode) {
        groupByAstNode.setTableName(NodeExpressionConverter.parseAndGetExpression(listIterator, getKeywordList()));
    }
    ...
    public String getKeywordTokenStr() {
        return SQLFoldingConstants.SQL_KEYWORD_DELETE;
    }
