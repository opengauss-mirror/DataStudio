 ...
 * @since 3.0.0
 */
public class CommandToggleLineComment {
    ...
    public void execute(@Named(IServiceConstants.ACTIVE_PART) @Optional MPart activePart) {
        ...
        if (partObject instanceof PLSourceEditor) {
            ((PLSourceEditor) partObject).getSourceEditorCore().toggleLineComment();
        } else if (partObject instanceof SQLTerminal) {
            ((SQLTerminal) partObject).getTerminalCore().toggleLineComment();
        }
    }
